...
export async function getUserInfo(torrentPage: cheerio.CheerioAPI): Promise<TPageUserInfo> {
  ...
  try {
    ...
    const [trash1, magicPoint] = magicPointContent.match(/\[使用\]:\s*(.*)\s*\(/)  || ['', ''];
    const [trash2, shareRatio] = magicPointContent.match(/分享率： (\d*\.*\d*)\s*/) || ['', ''];
    const [trash3, uploadCount] = magicPointContent.match(/上传量：\s*(\d*\.*\d*)\s*/)  || ['', ''];
    const [trash4, downloadCount] = magicPointContent.match(/下载量：\s*(\d*\.*\d*)\s*/)  || ['', ''];
...
export async function publishDate(el: cheerio.CheerioAPI): Promise<Date> {
  ...
  const dateString: string = await el('.rowfollow:nth-child(4) span').attr('title');
