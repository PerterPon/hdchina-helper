...
export async function getUserInfo(torrentPage: cheerio.CheerioAPI): Promise<TPageUserInfo> {
  ...
  try {
    ...
    const [trash1, magicPoint] = magicPointContent.match(/魔力值：\[使用\]\:\s(.*)\s邀请/)  || ['', ''];
    const [trash2, shareRatio] = magicPointContent.match(/分享率： (\d*\.*\d*)\s*/) || ['', ''];
    const [trash3, uploadCount] = magicPointContent.match(/上传量： (.*)\s下载/)  || ['', ''];
    const [trash4, downloadCount] = magicPointContent.match(/下载量： (.*)\s当前活动/)  || ['', ''];
    ...
    const nickAndUid = utils.fetchNicknameAndUidFromPage(torrentPage, '#info_block .bottom12:nth-child(1) .medium span a');
...
export async function getSiteId(el: cheerio.CheerioAPI, torrentUrl): Promise<string> {
  const idHref: string= el('.download').parent().attr('action');
...
export async function getTorrentUrl(el: cheerio.CheerioAPI): Promise<string> {
  const url = el('.download').parent().attr('action');
  return url;
}
...
export async function publishDate(el: cheerio.CheerioAPI): Promise<Date> {
  ...
  const dateString: string = await el('.rowfollow:nth-child(4)').attr('title');
...
export async function isSticky(el: cheerio.CheerioAPI): Promise<boolean> {
  ...
  const stickyFlag = await el('.top_icon');
