...
export async function getUserInfo(torrentPage: cheerio.CheerioAPI): Promise<TPageUserInfo> {
  ...
  try {
    ...
    const [trash1, magicPoint] = magicPointContent.match(/魔力值\s*\[使用\]\:\s(.*)\s\(/)  || ['', ''];
    const [trash2, shareRatio] = magicPointContent.match(/分享率： (\d*\.*\d*)\s*/) || ['', ''];
    const [trash3, uploadCount] = magicPointContent.match(/上傳量： (.*)\s下/)  || ['', ''];
    const [trash4, downloadCount] = magicPointContent.match(/下載量： (.*)當前活動/)  || ['', ''];
