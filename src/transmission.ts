...
export async function addTorrentUrl(url: string, serverId: number, fileId: string, torrentHash: string, tag: string, fileName: string): Promise<{transId: string; hash: string; serverId: number;}> {
  const serverConfig = getServerConfig(serverId);
  log.log(`[Transmission] addTorrentUrl: [${url}], server id: [${serverId}], download dir: [${serverConfig.fileDownloadPath}], fileId: [${fileId}], torrentHash: [${torrentHash}], tag: [${tag}], fileName: [${fileName}]`);

  const client: IClient = getServer(serverId);

  const curFileDownloadPath: string = path.join(serverConfig.fileDownloadPath, fileId);
  try {
    const addFunc = client.addTorrentUrl(url, curFileDownloadPath, torrentHash, tag, fileName);
    const res = await utils.timeout(
      addFunc,
      60 * 1000,
